...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/tarik1377/AccessLicense/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/tarik1377/AccessLicense/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/tarik1377/AccessLicense/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/tarik1377/AccessLicense/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/tarik1377/AccessLicense/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://github.com/tarik1377/AccessLicense/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_vless_reality_guide() {
    ...
    echo -e "${yellow}  │${plain}  Dest (SNI)   : ${blue}www.apple.com:443${plain}                    ${yellow}│${plain}"
    echo -e "${yellow}  │${plain}  Server Names : ${blue}www.apple.com${plain}                        ${yellow}│${plain}"
    echo -e "${yellow}  │${plain}                                                     ${yellow}│${plain}"
    echo -e "${yellow}  │${plain}  ${green}Good SNI targets (low latency, TLS 1.3, H2):${plain}       ${yellow}│${plain}"
    echo -e "${yellow}  │${plain}    - ${blue}www.apple.com${plain}                                   ${yellow}│${plain}"
    ...
    echo ""
    echo -e "${yellow}Step 8: Enable ML-DSA-65 (Post-Quantum Protection)${plain}"
    echo -e "  - In Reality settings, click ${blue}'Get New mldsa65 Seed'${plain}"
    echo -e "  - This adds post-quantum protection (parameter ${blue}pqv${plain} in links)"
    echo -e "  - ${green}Recommended${plain}: protects against future quantum decryption attacks"
    echo ""
    echo -e "${yellow}Step 9: Backup Transport — xHTTP${plain}"
    echo -e "  - If TCP gets blocked, switch Transport to ${blue}xHTTP${plain} in Inbound settings"
    echo -e "  - xHTTP splits data into HTTP POST/GET requests — looks like normal web browsing"
    echo -e "  - ${red}Note${plain}: Vision flow doesn't work with xHTTP, but xmux provides multiplexing"
    echo ""
    echo -e "${yellow}Step 10: Save and Connect${plain}"
    ...
    echo -e "  - ${blue}xtls-rprx-vision${plain} flow is mandatory for VLESS+Reality (TCP only)"
    echo -e "  - REALITY does NOT need any real SSL certificate"
    echo ""
    echo -e "${yellow}For High Load (many users):${plain}"
    echo -e "  - ${blue}bufferSize=0${plain} is set by default (no limit)"
    echo -e "  - ${blue}BBR${plain} congestion control is enabled"

...
VLESS_LINK="vless://${CLIENT_UUID}@${SERVER_IP}:${VLESS_PORT}?type=tcp&security=reality&pbk=${PUB_KEY}&fp=${FP}&sni=${SNI}&sid=${SHORT_ID}&flow=xtls-rprx-vision#${NODE_NAME}"
...
echo ""
log "  Подписки (рекомендуется — обновляются автоматически):"
...
log "═══════════════════════════════════════════════════════════"
info "  1. Добавь профиль по ССЫЛКЕ НА ПОДПИСКУ (не по VLESS ссылке)"
info "     Подписка автоматически включает fragment, noise, routing"
info "  2. Если не подключается — смени SNI в настройках профиля:"
info "       www.google.com / dl.google.com / www.apple.com"
info "  3. Попробуй DNS: https://common.dot.dns.yandex.net/dns-query"
log "═══════════════════════════════════════════════════════════"
echo ""
log "  Nginx:     порт 80 → сайт-прикрытие"
log "  Мониторинг:  панель управления"
log "============================================"
